...
mod test {
    ...
    use crate::cp::link::{PESubspaceSnark, SparseMatrix, SubspaceSnark, PP};
    ...
    fn test_basic() {
        ...
        let mut pp = PP::<Bls12_377> { l: 1, t: 2, g1, g2 };
        ...
        let (ek, vk) = PESubspaceSnark::<Bls12_377>::keygen(&mut rng, &pp, m);

        let pi = PESubspaceSnark::<Bls12_377>::prove(&mut pp, &ek, &x);
        let pi_bad = PESubspaceSnark::<Bls12_377>::prove(&mut pp, &ek, &x_bad);

        let b = PESubspaceSnark::<Bls12_377>::verify(&pp, &vk, &y, &pi);
        let b_bad = PESubspaceSnark::<Bls12_377>::verify(&pp, &vk, &y, &pi_bad);
        assert!(b);
        assert!(!b_bad);
    }
    ...
    fn test_same_value_different_bases() {
        ...
        let mut pp = PP::<Bls12_377> { l: 2, t: 3, g1, g2 };
        ...
        let y: Vec<G1Affine> = vec![
            bases1[0].mul(x[0]) + bases1[1].mul(x[2]),
            bases2[0].mul(x[1]) + bases2[1].mul(x[2]),
        ]
        .into_iter()
        .map(|p| p.into_affine())
        .collect::<Vec<_>>();
